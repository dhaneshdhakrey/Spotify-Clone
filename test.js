...
async function getsongs() {
    let response = await fetch("http://172.16.112.126:3000/songs/");
...
function playsong(id,pause=false,songs) {
  let track= getsonganame(songs,id)
...
function getname(string1){
    let name="";
    for (let i = 0; i < string1.length; i++) {
        if(string1[i]!==','){name+=string1[i]}
        else {break};
...
function getsonganame(songs, id) {
    let temp = "";
...
async function main(string1) {
    ...
    for (const song of songs) {
        ...
        <li>
                        ...
                        <div class="songinfo">
                        <div hidden ="true" class="songid">${setid(song)}</div>
                            <div >${getname(song)}</div>
                            <div>By Dhanesh</div>
    ...
    Array.from(document.querySelector(".songslist").getElementsByTagName("li")).forEach(e => {
        e.addEventListener("click", element => {
            ...
            playsong(e.querySelector(".songinfo").firstElementChild.innerHTML,false,songs)
        })
